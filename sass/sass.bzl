...
def _run_sass(ctx, input, css_output, map_output):
  ...
  # sassc requires --sourcemap to produce the .map file, or else the rule fails.
  # However dart sass only accepts --source-map (add'l hyphen)
  # because it's CLI-compatible with node-sass.
  if str(ctx.attr.compiler.label) == "@sassc//:sassc":
    args.add(["--sourcemap"])
